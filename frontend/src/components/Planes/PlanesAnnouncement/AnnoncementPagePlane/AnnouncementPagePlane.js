import "./AnnouncementPagePlane.css";
import React from 'react';
import { useEffect, useState } from "react"; 
import {useParams} from "react-router-dom";

export default function AnnouncementPagePlane() {
    const { id } = useParams();
    ...
    useEffect(() => {
        ...
        fetch(`http://localhost:8080/plane/${id}`)
    ...
    const { name, descr, typeplane, image, price } = announcementData;

    const sendMessage = () => {
        // Замените номер телефона и сообщение своими значениями
        const phoneNumber = '87053245524'; // Замените на номер получателя
        const message = `Здравствуйте, меня интересует ${name} (${typeplane}) за ${price} ₸. Пожалуйста, предоставьте более подробную информацию.`;
    ...
    <div className="announcement-cont">
        ...
        <img src={image} alt="" className="main-img"/>
             <div className="announcement-type">
                ...
                <p className="typeof"> {typeplane}</p>
            </div>
        </div>      
